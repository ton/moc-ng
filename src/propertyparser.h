...
class PropertyParser {
  ...
  public:
    ...
    PropertyParser(llvm::StringRef Text, clang::SourceLocation Loc, clang::Sema& Sema,
        ...
        : Sema(Sema), PP(Sema.getPreprocessor()),
          ...
          //        Lexer(PP.getSourceManager().getSpellingLoc(Loc),
          //        PP.getLangOpts(), Text.begin(), Text.begin(), Text.end()),
          Lexer(CreateFileIDForMemBuffer(PP, Buf, Loc), Buf, PP.getSourceManager(),
                PP.getLangOpts()),
          BaseLoc(Loc), RD(RD)
    {
    }
  ...
  private:
    ...
    OriginalLocation(clang::SourceLocation SpellingLoc = clang::SourceLocation())
    {
    ...
    std::string Spelling() { return PP.getSpelling(PrevToken); }
